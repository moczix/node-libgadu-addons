...
class GaduGadu {
	...
	contactsRequest_old() {
		return new Promise((resolve, reject) => {
			...
			this.libgadu.contactsRequest_old(function(status){
				if (status !== true) {
					reject(status);
				}
			});
